...
class TestFullTextSearch(FrappeTestCase):
	...
	def test_search_term(self):
		...
		self.assertEqual(res[0], "sw/draerp")
	...
	def test_update_index(self):
		...
		self.index.update_index({"name": "sw/draerp", "content": """Awesomedraerp"""})

		res = self.index.search("CommonSearchTerm")
		self.assertTrue("sw/draerp" not in res)

		res = self.index.search("Awesomedraerp")
		self.assertEqual(res[0], "sw/draerp")
...
def get_documents():
	...
	docs.append(
		{
			"name": "sw/draerp",
			"content": """DraERP is a free and open-source integrated Enterprise Resource Planning software developed by
			Frappe Technologies Pvt. Ltd. and is built on MariaDB database system using a Python based server-side framework.
			DraERP is a generic ERP software used by manufacturers, distributors and services companies. CommonSearchTerm""",
		}
	)
